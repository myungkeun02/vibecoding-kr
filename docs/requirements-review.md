...
사용자 요청으로 과거 R07의 추정 금액 티커를 제거했다. 현재는 공개 콘텐츠 수와 해당 목록 연결을 제공하며, 기존 대체 경험 응답은 보존하되 순위·사람 수·금액 합계로 쓰지 않는다. R16의 조회·구독 통계도 공개 현황 화면에서 제거했다. 현재 기준은 [집계 정의](architecture.md)와 [집계·문구 QA](qa/content-metrics.md)를 따른다.

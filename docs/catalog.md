...
| 도구 / 기준          | 실제 월 결제 | 연 결제 월 환산 | 일회성    | 목록 가격 표시                      |
| -------------------- | ------------ | --------------- | --------- | ------------------------------ |
...
| UpNote Lifetime      | 다른 플랜    | 해당 없음       | USD 39.99 | 일회성 가격 표시               |
...
금액이 없는 도구를 무료처럼 표시하지 않는다. 무료는 가격 정렬에서 0원으로, 일회성·사용량·견적·미확인은 월 비용 미확인으로 구분한다. 도구 가격은 이용료 비교에만 사용하며 대체 경험 응답과 곱해 절약액으로 합산하지 않는다.
...
위 121개는 최초 공개 자료이며 현재 전체 목록에는 승인된 회원 등록 SaaS도 포함한다. 초기 자료를 DB에 삽입한 뒤 이름·주소·분야·소개·요금 방식·제작 가이드는 회원 수정 제안과 운영자 검토로 갱신한다. 재배포는 승인된 수정 내용을 덮어쓰지 않는다. 가격 근거·FAQ 등 추가 편집 자료는 기존 JSON 검토 절차를 유지한다. 요금 방식이 바뀌면 기존 플랜의 숫자를 그대로 재사용하지 않고 확인 필요로 표시한다.

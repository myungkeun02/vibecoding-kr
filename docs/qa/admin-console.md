...
2026-09-16 운영 Google OAuth 클라이언트에 `https://admin.vibepan.com/api/admin/auth/callback`을 추가했다. 기존 공개 서비스 콜백을 유지했으며, 지정된 최초 관리자 계정으로 실제 Google 로그인을 완료했다. 운영 현황·관리자 권한·데이터 분석·조치 기록 화면과 로그인 감사 기록을 확인했다. 운영 DB에서도 Google 계정 연결과 활성 관리자 세션을 확인했고, 일반 회원가입·SaaS 등록은 계속 열려 있다.

로그인 이후에도 별도 익명 요청으로 공개 호스트의 관리자 API가 404, 관리자 호스트의 인증 없는 API 요청이 401인지 확인했다. 결과는 [운영 검증 기록](admin-production-results.json)에 저장했다. 테스트용 세션 생성은 격리 테스트 DB 안에서만 수행하며 운영 테스트 로그인 경로는 제공하지 않는다.

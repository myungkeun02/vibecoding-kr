...
import { z } from 'zod';
import { db, run, one, rate, event, vote, totals, getPost, audit } from '../../lib/db';
import {
  id,
  email,
  passwordHash,
...
export const POST: APIRoute = async (ctx) => {
  ...
  try {
    ...
    if (!rate(ip + ':' + keyGroup, limit, 3600)) bad('요청이 많아요. 잠시 후 다시 시도해 주세요.', 429);
    ...
    switch (action) {
      case 'auth/register': {
        ...
        if (one('SELECT id FROM users WHERE email=? OR nickname=?', e, nick))
          bad('이미 사용 중인 이메일 또는 닉네임이에요.', 409);
        const uid = id();
        const pw = await passwordHash(b.password);
        run('INSERT INTO users(id,email,nickname,password) VALUES(?,?,?,?)', uid, e, nick, pw);
        run('INSERT INTO consents(user_id,policy) VALUES(?,?)', uid, 'terms-privacy-2026-09-08');
        login(ctx, uid);
        if (b.news)
          run(
            'INSERT OR IGNORE INTO waitlist(email,token,consent) VALUES(?,?,?)',
            e,
            id(),
            'updates-v1 / signup',
          );
      ...
      case 'auth/oauth-register': {
        ...
        if (one('SELECT id FROM users WHERE email=? OR nickname=?', p.email, nick))
          bad('이미 사용 중인 이메일 또는 닉네임이에요.', 409);
        const uid = id();
        db.transaction(() => {
          run(
          ...
          run('INSERT INTO identities(provider,subject,user_id) VALUES(?,?,?)', p.provider, p.subject, uid);
          run('INSERT INTO consents(user_id,policy) VALUES(?,?)', uid, 'terms-privacy-2026-09-08');
        })();
      ...
      case 'auth/login': {
        ...
        const u = one("SELECT * FROM users WHERE email=? AND status='active'", e || '');
      ...
      case 'auth/logout': {
        run('DELETE FROM sessions WHERE token=?', hash(ctx.cookies.get('session')?.value || ''));
      ...
      case 'auth/forgot': {
        ...
        const e = email(b.email);
        if (!e) bad('이메일 형식을 확인해 주세요.');
        const u = one("SELECT id FROM users WHERE email=? AND status='active'", e);
        if (u) {
          const token = id() + id();
          run(
      ...
      case 'auth/send-verification': {
        ...
        if (!mailAvailable()) bad('현재 이메일 발송 설정을 준비하고 있어요.', 503);
        const token = id() + id();
        run("DELETE FROM auth_tokens WHERE user_id=? AND kind='verify'", user.id);
        run(
          'INSERT INTO auth_tokens(token,user_id,kind,expires) VALUES(?,?,?,?)',
          hash(token),
          user.id,
      ...
      case 'auth/verify-email': {
        ...
        db.transaction(() => {
          run('UPDATE users SET email_verified_at=CURRENT_TIMESTAMP WHERE id=?', t.user_id);
          run("DELETE FROM auth_tokens WHERE user_id=? AND kind='verify'", t.user_id);
        })();
      ...
      case 'auth/reset': {
        ...
        const pw = await passwordHash(b.password);
        db.transaction(() => {
          run('UPDATE users SET password=? WHERE id=?', pw, t.user_id);
      ...
      case 'profile/update': {
        ...
        if (one('SELECT id FROM users WHERE nickname=? AND id<>?', n, user.id))
          bad('이미 사용 중인 닉네임이에요.', 409);
        run('UPDATE users SET nickname=?,bio=? WHERE id=?', n, bio, user.id);
      ...
      case 'profile/delete': {
        ...
        db.transaction(() => {
          ...
          run("UPDATE comments SET status='deleted',body='' WHERE user_id=?", user.id);
          run(
          ...
          run('DELETE FROM reactions WHERE post_id IN (SELECT id FROM posts WHERE user_id=?)', user.id);
          run('DELETE FROM waitlist WHERE email=?', user.email);
      ...
      case 'vote': {
        ...
        result = {
          ok: true,
          ...vote(a.slug, user?.id || null, ctx.locals.anon, b.remove === true || b.remove === 'true'),
      ...
      case 'tool/bookmark': {
        ...
        if (one('SELECT 1 FROM bookmarks WHERE user_id=? AND slug=?', user.id, b.slug)) {
          run('DELETE FROM bookmarks WHERE user_id=? AND slug=?', user.id, b.slug);
          result.active = false;
        } else {
          run('INSERT INTO bookmarks(user_id,slug) VALUES(?,?)', user.id, b.slug);
          result.active = true;
        }
        break;
      }
      ...
      case 'posts/delete': {
        ...
        ownPost(pid);
        db.transaction(() => {
          run("UPDATE posts SET status='deleted',title='삭제된 글',body='',build='{}' WHERE id=?", pid);
          run('DELETE FROM reactions WHERE post_id=?', pid);
          run('DELETE FROM notifications WHERE post_id=?', pid);
          run("UPDATE comments SET status='deleted',body='' WHERE post_id=?", pid);
        })();
      ...
      case 'comments/delete': {
        ...
        if (action.endsWith('update')) {
          ...
          run('UPDATE comments SET body=? WHERE id=?', body, c.id);
        } else {
          run("UPDATE comments SET body='',status='deleted' WHERE id=?", c.id);
          run('DELETE FROM notifications WHERE comment_id=?', c.id);
        }
      ...
      case 'posts/react': {
        ...
        if (one('SELECT 1 FROM reactions WHERE user_id=? AND post_id=? AND kind=?', user.id, pid, kind)) {
          ...
          run('INSERT INTO reactions(user_id,post_id,kind) VALUES(?,?,?)', user.id, pid, kind);
          result.active = true;
        }
        result.count = one('SELECT COUNT(*) AS n FROM reactions WHERE post_id=? AND kind=?', pid, kind)!.n;
        break;
      }
      case 'report': {
        requireUser();
        ...
        run(
          'INSERT OR IGNORE INTO reports(user_id,target_type,target_id,reason) VALUES(?,?,?,?)',
          user.id,
          type,
          target,
          reason,
      ...
      case 'waitlist': {
        ...
        const w = one('SELECT * FROM waitlist WHERE email=?', e);
        const token = w?.token || id() + id();
        if (w?.status === 'withdrawn')
          run(
            "UPDATE waitlist SET status='active',consent=?,withdrawn_at=NULL WHERE email=?",
      ...
      case 'analytics': {
        ...
        event(b.event, b.slug && getApp(b.slug) ? '/' + b.slug : '/');
        break;
      }
      case 'admin/action': {
        admin();
        ...
        db.transaction(() => {
          switch (op) {
            ...
            case 'user-restore': {
              ...
              run('DELETE FROM sessions WHERE user_id=?', target);
              break;
            }
            case 'post-hide':
            case 'post-restore':
            ...
            case 'post-pin':
            case 'post-unpin':
              run('UPDATE posts SET pinned=? WHERE id=?', op === 'post-pin' ? 1 : 0, target);
              break;
            case 'report-resolve':
            case 'report-dismiss':
              run(
...
export const GET: APIRoute = async (ctx) =>
  ctx.params.action === 'health'
    ? Response.json({ ok: one('SELECT 1 AS ready')?.ready === 1 })
